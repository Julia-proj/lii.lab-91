...
export async function PATCH(
  ...
  try {
    ...
    if (msg === 'Reserva no encontrada') return NextResponse.json({ error: msg }, { status: 404 })
    if (msg === 'Ese horario ya está ocupado') return NextResponse.json({ error: msg }, { status: 409 })
